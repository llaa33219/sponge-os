...
| 14 | `(see subject)` | feat(genode): base-sel4 high-phys frame-cap CNode for IO_MEM at phys_addr >= 8 GiB | `genode/repos/base-sel4/src/core/`: `include/core_cspace.h` adds `NUM_HIGH_PHYS_SEL_LOG2 = 23`, `HIGH_PHYS_BASE = 1ULL << 33` (8 GiB), top-CNode slot `TOP_CNODE_HIGH_PHYS_IDX = 0x7e0`, `high_phys_cnode_sel()`; `include/untyped_memory.h` adds `_high_phys_sel()` and `_high_phys_untyped_sel()` (bootinfo scan for the covering device untyped) plus high-phys branches in the frame-cap paths; `include/platform.h` + `platform.cc` add `Constructible<Cnode> _high_phys_cnode` and `construct_high_phys_cnode()` (lazy 256 MiB backing allocated from the 16 KiB pool on first demand, cap copied to top slot 0x7e0), register high-phys device memory in `_io_mem_alloc` WITHOUT eager retype (whole untyped on the first batch — per-batch registration would explode the AVL tree with ~500k 64-KiB nodes per 32 GiB untyped and hang the boot); `io_mem_session_support.cc::_acquire` triggers the lazy construction before the high-phys retype; `include/vm_space.h` + `spec/x86/vm_space.cc` add `frame_source_cnode()`/`frame_source_index()` (slot = (phys - HIGH_PHYS_BASE) >> 12) used by `_map_frame`; `include/initial_untyped_pool.h` passes the Range into the conversion functors. NOTE: the 16 KiB pool reservation in `spec/x86_64/platform.cc` is deliberately UNCHANGED — an eager 256 MiB carve for the CNode backing regressed the AHCI desktop on 4 GiB VMs (`alpha-probe: sponge_pkgd did not answer install hello`); the backing is allocated lazily at demand time instead (early boot, contiguous RAM plentiful; failure mode is a clean IO_MEM "not available", not a crash). | Row 13 makes the seL4 kernel publish device untypeds above the memory map, but Genode core's flat phys CNode (2^21 slots x 4 KiB = 8 GiB of physical coverage) could not store frame caps for phys >= 8 GiB — the "limited untyped cnode range" warnings and the `I/O memory [0x800000000,...) not available` failure (xHCI BAR at 32 GiB on q35/OVMF; same class as the Insyde GOP FB on the 17ZD90N). A flat-index bump is infeasible (1 TiB at 4 KiB granularity = 8 GiB of CNode backing), so a second CNode covers [8 GiB, 40 GiB) at 4 KiB granularity, constructed lazily on first IO_MEM demand. Verified: `run/sponge-desktop-disk-uefi-usb` with `nec-usb-xhci` (BAR at 32 GiB) reaches `sponge_configd: ready`; regressions `sponge-minimal`, `sponge-fbprobe-uefi`, `sponge-desktop-disk` (alpha-probe PASS) all green. | Upstream seL4/Genode grows a proper high-MMIO IO_MEM path (large-page device frames or a windowed phys CNode scheme). |
