...
| 11 | `87c58ac9ae` | fix(genode): sel4 kernel: fix reserved/avail overlap subtraction in init_freemem for fragmented UEFI maps | `genode/contrib/sel4-*/src/kernel/sel4/src/kernel/boot.c` `init_freemem()` via `docs/patches/sel4-uefi-untyped-overlap.patch`: in the region-subtraction loop, only advance `r` (the reserved pointer) when the reserved region is fully consumed within the current avail region (`reserved[r].end <= avail_reg[a].end`); upstream advanced `r` unconditionally after trimming. | Under UEFI the kernel+userland reserved block spans MANY fragmented avail regions; the unconditional `r++` skipped subtracting the reserved block from the later fragments, leaking them into freemem. The result was OVERLAPPING untyped caps (e.g. a 2 MiB `[0x800000,0xa00000)` untyped overlapping `[0x80c000,0x810000)` and `[0x900000,0xa00000)`), which crashed the kernel (null deref in `decodeInvocation`) during Genode core's `Untyped_Retype`. BIOS maps don't fragment here so the bug never fired. Verified: OVMF Tier-0 smoke now reaches `boot-probe: PASS` with zero registration warnings, and the desktop chain reaches the Genode banner. Diagnosis: 15-3 bring-up log rounds 9–14. NOTE: `genode/contrib/` is not git-tracked; the patch file is the durable record, re-applied on `prepare_port sel4`. | Upstream seL4 fixes `init_freemem`'s reserved/avail subtraction for overlapping multi-fragment reserved regions. |
