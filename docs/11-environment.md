...
| 11 | `(this commit)` | refactor(genode): sel4: replace the init_freemem kernel fix with a reserved-extent export + core-side artifact detection | Kernel side: `genode/contrib/sel4-*/src/kernel/sel4/src/arch/x86/kernel/boot.c` via `docs/patches/sel4-reserved-extent-export.patch` (~30 lines): `init_sys_state()` appends ONE synthetic multiboot-mmap entry (type 2, reserved) covering `[KERNEL_ELF_PADDR_BASE, ui_info.p_reg.end)` — the kernel image + boot-module blob + copied rootserver image — into the otherwise-empty (under multiboot2) X86_MBMMAP bootinfo chunk. **The kernel only exports the fact; all policy lives in userland** (the microkernel-purity principle, AGENTS §1.2/§5.2). Core side: `genode/repos/base-sel4/src/core/include/initial_untyped_pool.h` (already a row-6/7 patched file) gains a lazy one-shot `_detect_artifacts_once()` that flags RAM untypeds which (1) overlap another RAM untyped (freemem regions are disjoint by construction — any overlap is a subtraction artifact), (2) overlap core's own image, physical base queried from the kernel via `seL4_X86_Page_GetAddress(bi.userImageFrames.start)` (loader-placement independent; linked-extent fallback), or (3) overlap a non-usable mmap entry (the synthetic reserved extent from the kernel export, plus real firmware-reserved regions on multiboot1 boots). Flagged ranges are skipped in both `turn_into_untyped_object()` (covers the 16 KiB-pool path AND the 4 KiB phys-CNode path — detection must live at pool level because the 16K pool runs before `_init_allocators`) and `alloc()`. Why: the seL4 kernel's `init_freemem` subtracts the reserved extent from the available regions with a pointer that advances unconditionally per reserved entry; on fragmented maps (OVMF, Insyde H2O) the later avail fragments are never subtracted, leak into freemem, and surface as RAM untypeds over the live kernel/image (e.g. the 9-warning set `[0x200000,0x4000000)` on the OVMF desktop boot). Retyping them corrupts kernel state (historical crash: null deref in `decodeInvocation` during `Untyped_Retype`; on the 26.08 OVMF boot the crash moved into the 16K-pool retype — the artifacts were being registered as CNode backing over the kernel image). The artifacts are deliberately LEAKED (never retyped, never allocated from) — a few MiB of unusable fragments on fragmented boots, the price of a pristine kernel. Replaces the former row-11 subtraction-loop patch (`87c58ac9ae`, `docs/patches/sel4-uefi-untyped-overlap.patch`, DELETED): under multiboot2 the old patch's silent-fix-in-kernel approach also masked the fact that core never learns WHICH memory is reserved; the export approach is a strictly smaller kernel diff (~30 export-only lines vs a ~20-line surgery inside the subtraction loop) and survives upstream refactors of that loop. NOTE: `genode/contrib/` is not git-tracked; the patch file is the durable record, re-applied on `prepare_port sel4`. | Verified on 26.08 (KERNEL=sel4 BOARD=pc): `run/sponge-desktop-disk-uefi-usb` PASS (3/3 structural gates + OVMF UEFI boot + usb_block storage chain; 9 artifacts detected+skipped per boot), `run/sponge-minimal` PASS (3 artifacts), `run/sponge-configd-persist` PASS. The development history (two failed approaches: identity-assumed image extent, ELF-phdr extent) is recorded in `docs/evidence/row11-core-side-migration-attempt.md`. | Upstream seL4 fixes `init_freemem`'s reserved/avail subtraction AND exports the reserved extent (or an equivalent) in the bootinfo; then this row's core-side detection becomes pure defense-in-depth and the kernel export patch can be dropped. |
