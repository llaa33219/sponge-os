...
| `11f4d3ad8a` | feat(genode): vendor Genode 26.05 as git subtree at genode/ |
| `7255374d0d` | fix(genode): stdcxx port: replace dead gcc mirror with ftp.gnu.org |
| `07c0fe3138` | build(genode): disable qttools in qt6 host-tools build |
| `a42e2d28f4` | build(genode): parameterize QT_TOOLS_DIR with repo-local default |
| `2909e8b57b` | fix(genode): qt6 cmake build: block host-header leakage via sponge-owned wrappers |
| `4e91a4a602` | feat(tool): add sponge-owned compiler wrappers + wire repos/sponge into vendored tree |
| `b27d2be34e` | fix(run): replace absolute run-script symlinks with relative ones |
| `042a7003ae` | chore: remove vendored genode/ for re-import onto the canonical upstream lineage |
| `829bb83a54` | Add 'genode/' from commit 'd18ee9e917200b059549e21f2dd9a55bfe61a0eb' — Genode 26.08 re-vendor (canonical lineage) |
...
├── genode/                     # Vendored Genode 26.08 (git subtree, pinned)
...
│   └── Makefile, etc/, ...     # upstream Genode 26.08 contents
├── repos/
...
|-----------|---------|-------------------|------------------------|
| **Genode OS Framework** | 26.08 | upstream commit `d18ee9e917200b059549e21f2dd9a55bfe61a0eb` (`codeberg.org/genodelabs/genode` tag `26.08^{}`) | vendored at `genode/` via `git subtree`; the 26.05→26.08 step was a remove+re-add re-import (the old subtree sat on a mirror side-lineage whose merge-base predated 26.05, so `subtree pull` would not merge); future upgrades via `git subtree pull --prefix=genode` (recorded as a dedicated commit) |
| **seL4 kernel** | 16.0.0 (Sponge override) | upstream commit `6e7c3b733d296cfd88d5fbf635c96e447a882374` (github.com/seL4/seL4 tag `16.0.0`), pinned by `genode/repos/base-sel4/ports/sel4.port` (`REV(sel4)`, port hash `a81460876b917293b45aa18015dc117429f7ffe9`) | **Upstream Genode 26.08 pins 13.0.0 (Jul 2024, `cd6d3b8c`) — 3 major releases behind.** Sponge overrides the pin to 16.0.0 (Jul 2026) to pick up upstream fixes (`4018ad594` fixes the init_freemem reserved-region subtraction leak — the bug our row-11 export+detection defends against; `aa329f947` handles mb2 unaddressable memory; `f34dab579` fixes an off-by-one). Genode's own base-sel4 patches were rebased for 16.0.0 (2 patches touched: `intel_vmx_disable_vpid` — upstream typo fix `degredation`→`degradation` in the context; `intel_vmx_full_state` — upstream renamed `SEL4_VMENTER_CALL_CONTROL_ENTRY_MR`→`CALL_INTERRUPT_INFO_MR` and split the line); core-side adaptation: `vm_space.cc` casts to the split `seL4_X86_EPT_VMAttributes` type. When Genode themselves bump the seL4 pin, the override collapses back to theirs. |
...
The vendored tree at `genode/` is **Genode 26.08 pristine** plus the
...
|---|--------|---------|--------------|-----|-----------|
| 1 | `7255374d0d` | fix(genode): stdcxx port: replace dead gcc mirror with ftp.gnu.org | `genode/repos/libports/ports/stdcxx.port`: `URL(gcc)` switched from `http://ftp.fu-berlin.de/...` to `https://ftp.gnu.org/gnu/gcc/...`; `.hash` updated to match | The `ftp.fu-berlin.de` mirror stopped serving the GCC tarball, so `prepare_port stdcxx` failed at the download step on a fresh machine. | Upstream updates the `stdcxx.port` URL itself, which would re-bump the hash. |
| 2 | `07c0fe3138` | build(genode): disable qttools in qt6 host-tools build | `genode/tool/tool_chain_qt6`: `BUILD_qttools=ON` switched to `OFF` | `qttools` is unneeded for the Core/Gui/Widgets-only DE Sponge OS builds, and was the failing part of the host-tools build on a fresh machine. | Sponge OS gains a Qt tools dependency (Designer, Linguist, …) at which point upstream's default is fine. |
| 3 | `a42e2d28f4` | build(genode): parameterize QT_TOOLS_DIR with repo-local default | `genode/repos/libports/lib/import/import-qt6.inc`: `QT_TOOLS_DIR ?=` pointing at `<repo>/var/qt6-host-tools`. Amended by a follow-up commit: the default is derived from the file's own location (`$(dir $(realpath ...))/../../../../../var/qt6-host-tools`) instead of `$(GENODE_DIR)`, because `GENODE_DIR` is not exported to per-target sub-makes and would expand to `/var/qt6-host-tools` there. | Upstream hardcodes `/usr/local/genode/tool/25.05/qt6.8.3`, which does not exist when the host tools are built from source into the repo-local cache. | Upstream parameterizes `QT_TOOLS_DIR`. |
| 4 | `2909e8b57b` | fix(genode): qt6 cmake build: block host-header leakage via sponge-owned wrappers | `genode/repos/libports/src/qt6/base/target.mk` (cmake invocation): compiler pointed at the wrappers `repos/sponge/tool/genode-x86-{gcc,g++}-wrapper`, which strip leaked host `-I/-isystem /usr/include` and `/usr/local/include` paths; clear `CMAKE_C_IMPLICIT_INCLUDE_DIRECTORIES` / `CMAKE_CXX_IMPLICIT_INCLUDE_DIRECTORIES`; add `-D__GNUCLIKE_BUILTIN_STDARG` and `-include sys/cdefs.h` for FreeBSD-derived libc headers; use qtbase's bundled `md4c` include; disable `system_doubleconversion`, `system_md4c`, `textmarkdownreader`, `textmarkdownwriter` Qt features. Amended by a follow-up commit: the wrapper directory is derived from the file's own location (`SPONGE_TOOL_DIR`) instead of `$(GENODE_DIR)` — same sub-make visibility reason as patch #3. | CMake's compiler introspection leaked host include paths into the Qt6 cross-build, breaking it against Genode headers, and the disabled features pulled in third-party dependencies that are not available on the Genode side. | Upstream either fixes the cmake invocation to never see host headers, or Sponge OS no longer uses Qt6 (unlikely). |
| 5 | `4e91a4a602` | feat(tool): add sponge-owned compiler wrappers + wire repos/sponge into vendored tree | new files `repos/sponge/tool/genode-x86-gcc-wrapper` and `genode-x86-g++-wrapper` (the bash filter scripts patch #4 leans on); `genode/repos/sponge` relative symlink to `../../repos/sponge` | The wrappers used to live in `/tmp/opencode/bin/` (a `tmpfiles`-swept path). Reboot wiped them and broke the build with a generic "no such compiler" error. Putting them under version control makes them part of the reproducible environment. | Never (these are Sponge-owned artifacts, not upstream patches). |
| 6 | `ea8ac1343b` | feat(genode): sel4: size-aware on-demand CNode backing for large child-PD CSpaces | `genode/repos/base-sel4/`: `src/core/include/cnode.h` (CNode ctor allocates backing sized to the requested CNode instead of one 4 KiB page), `src/core/include/untyped_memory.h` (size-aware untyped selector addressing), `src/include/base/internal/capability_space_sel4.h` (enlarged per-PD CSpace), `src/core/spec/{x86_64,arm,arm_v8a}/platform.cc` (allocator wiring; x86_64 path proven, other arches kept upstream-shaped) | Falkon-class dynamic workloads exhaust the fixed 8192-slot per-PD CSpace (each child-PD CNode backed by exactly one 4 KiB untyped page → 128 slots at seL4's 32-byte CTE). The fix makes the backing size-aware and on-demand, WITHOUT the fixed 16K-pool carve that regressed ahci DMA (diagnosis + canaries in `docs/evidence/c1-dma-safe-backing.log`). | Upstream resolves the long-standing `platform.cc:108` `XXX` ("allocate intermediate CNodes ... here") with dynamic CNode allocation. **Phase-15 addendum:** CSPACE_SIZE_LOG2_1ST raised 6->7 (x86_64) — 65536 slots/PD — after the 17ZD90N real-hw boot exhausted the vfs/rom_lib PDs' page-table-registry caps (slow real-USB I/O inflates the concurrently-live mapping set; the OVMF runs already grazed the 32768 limit) and stalled in the mapping-cache flush; the 16 KiB pool reservation stays pinned at the proven 64-CNodes/PD magnitude (decoupled from the geometry) to avoid the known 4 GiB desktop regression. |
| 7 | `38a19fa5d8` | feat(genode): sel4: lazy vm_space lower-level CNode growth (131072 mappings/PD) | `genode/repos/base-sel4/`: `src/core/include/vm_space.h` (`LEAF_CNODE_SIZE_LOG2` 7→9 on x86_64 → `NUM_VM_SEL_LOG2` 15→17; `_ensure_leaf` constructs 3rd/4th-level CNodes lazily on first use; no-op-safe destruct), `src/core/platform.cc` (core virt reservation capped to 256 MiB), `src/core/spec/x86_64/platform.cc` (16K pool +32 MiB for lazy vm leaves) | The per-PD mapping pool was a static 32768-entry tree (`NUM_VM_SEL_LOG2=15`) that falkon (~100k frames) and rom_pkg (~130k) both exhaust ("mapping cache full / out of selector"). Eager enlargement starved core heap or crashed Platform(); the fix constructs lower-level CNodes lazily on first use, growing to 131072 entries without boot-time pressure (canaries in `docs/evidence/c2-lazy-vmspace.log`). | Upstream resolves `platform.cc:108` `XXX` / grows vm_space dynamically. |
| 8 | `dc4a9f342f` | fix(genode): bash/ncurses ports: replace 502-ing ftpmirror.gnu.org with ftp.gnu.org | `genode/repos/ports/ports/bash.port` and `genode/repos/libports/ports/ncurses.port`: `URL(...)` switched from `https://ftpmirror.gnu.org/...` to `https://ftp.gnu.org/gnu/...`; the two `.hash` files regenerated via `tool/ports/update_hash`. **26.08 re-vendor extension (2026-09-12):** the same fix was applied to nine more GNU ports — `gmp`, `libiconv` (libports), `coreutils`, `diffutils`, `findutils`, `grep`, `sed`, `tar`, `which` (ports) — each with `update_hash` regeneration, because the `ftpmirror.gnu.org` redirector 502'd on all of them during the upgrade re-prepare | `ftpmirror.gnu.org` answers 502 Bad Gateway (mirror redirector outage), so `prepare_port bash ncurses` failed at the download step on a fresh machine; `ftp.gnu.org` serves the identical tarballs (SHA-256 unchanged). Same failure class as patch #1. | Upstream switches the two ports to a working mirror itself (which would re-bump the hashes). |
| 9 | `b4aa1d774f` | fix(genode): nitpicker: track pointer on relative motion before first absolute event | `genode/repos/os/src/server/nitpicker/user_state.cc`: the relative→absolute motion conversion's `Nowhere` branch (previously dropped the event) now initializes the pointer from the origin and lets the sanitizer clamp it | On PS/2-only machines (QEMU SDL/GTK, most real hardware) no absolute-motion event ever arrives, so `_pointer` stayed `Nowhere` forever: every relative motion was silently dropped — no cursor movement, misrouted clicks. Discovered when wiring the `pointer` cursor component into the product media (Phase 11 follow-up). | Upstream initializes `_pointer` for relative-only input paths (roadmap §11.2 item 2 already records the adjacent report-ROM gap). |
| 10 | `3b12a06cfb` | fix(genode): bender: tolerate reclaimable UEFI memmap overlaps + union-coverage RAM check | `genode/tool/boot/bender` (binary) rebuilt from `alex-ab/morbo` branch `genode_bender` @ `77a6918` via `docs/patches/bender-uefi-memmap.patch`: `standalone/mbi2.c` `check_mem()` now (a) tolerates phdr overlaps with ACPI-reclaim (type 3) / NVS (type 4) regions (reusable after ExitBootServices; OVMF puts an NVS region at 0x800000 overlapping the seL4 kernel phdr), keeping type 2/5 as hard errors, and (b) replaces the single-region-containment `in_ram` check with union coverage over usable regions (UEFI fragmentation straddles the phdr across a type-1/type-4 boundary). Build env: host gcc -m32 + source-built nasm 2.16.03 + SCons (see patch for the modern-GCC/SConscript adjustments). | The stock bender refused every UEFI boot at check_mem (W1 `Reserved memory 800000+8000 type=4 overlaps with phdr ...` / `phdr ... is outside RAM !`). The W1 5-byte NOP bypass was over-broad (disabled all of check_mem); this is the principled fix. It does NOT resolve the downstream core crash (15-3 bring-up log round 10). | Upstream Genode ships a bender whose check_mem understands UEFI-reclaimable regions, or the boot chain no longer needs bender. |
| 11 | `9d21147962` | refactor(genode): sel4: replace the init_freemem kernel fix with a reserved-extent export + core-side artifact detection | Kernel side: `genode/contrib/sel4-*/src/kernel/sel4/src/arch/x86/kernel/boot.c` via `docs/patches/sel4-reserved-extent-export.patch` (~30 lines): `init_sys_state()` appends ONE synthetic multiboot-mmap entry (type 2, reserved) covering `[KERNEL_ELF_PADDR_BASE, ui_info.p_reg.end)` — the kernel image + boot-module blob + copied rootserver image — into the otherwise-empty (under multiboot2) X86_MBMMAP bootinfo chunk. **The kernel only exports the fact; all policy lives in userland** (the microkernel-purity principle, AGENTS §1.2/§5.2). Core side: `genode/repos/base-sel4/src/core/include/initial_untyped_pool.h` (already a row-6/7 patched file) gains a lazy one-shot `_detect_artifacts_once()` that flags RAM untypeds which (1) overlap another RAM untyped (freemem regions are disjoint by construction — any overlap is a subtraction artifact), (2) overlap core's own image, physical base queried from the kernel via `seL4_X86_Page_GetAddress(bi.userImageFrames.start)` (loader-placement independent; linked-extent fallback), or (3) overlap a non-usable mmap entry (the synthetic reserved extent from the kernel export, plus real firmware-reserved regions on multiboot1 boots). Flagged ranges are skipped in both `turn_into_untyped_object()` (covers the 16 KiB-pool path AND the 4 KiB phys-CNode path — detection must live at pool level because the 16K pool runs before `_init_allocators`) and `alloc()`. Why: the seL4 kernel's `init_freemem` subtracts the reserved extent from the available regions with a pointer that advances unconditionally per reserved entry; on fragmented maps (OVMF, Insyde H2O) the later avail fragments are never subtracted, leak into freemem, and surface as RAM untypeds over the live kernel/image. Retyping them corrupts kernel state (historical crash: null deref in `decodeInvocation` during `Untyped_Retype`). The artifacts are deliberately LEAKED (never retyped, never allocated from). Replaces the former row-11 subtraction-loop patch (`87c58ac9ae`, DELETED). **seL4 16.0.0 addendum:** upstream commit `4018ad594` ("boot: fix init_freemem skipping reserved regions") fixed the subtraction loop itself, so on 16.0.0 the leak shrinks from 9 artifacts/boot to 3 (only the kernel-window low-memory fragments remain); this row's export+detection now runs as defense-in-depth on top of the fixed kernel, catching the residual class. Patch rebased onto 16.0.0 (regenerated; the vtx_init context hunk removed — that change belongs to Genode's `intel_vtx_check.patch`). NOTE: `genode/contrib/` is not git-tracked; the patch file is the durable record, re-applied on `prepare_port sel4` + manual application. | Verified on 26.08 + seL4 16.0.0 (KERNEL=sel4 BOARD=pc): `run/sponge-desktop-disk-uefi-usb` PASS (3/3 structural gates + OVMF UEFI boot + usb_block storage chain; 3 residual artifacts detected+skipped per boot), `run/sponge-minimal` PASS (3 artifacts), `run/sponge-configd-persist` PASS. Development history in `docs/evidence/row11-core-side-migration-attempt.md`. | Upstream seL4 exports the reserved extent (or equivalent) in the bootinfo natively; then the export patch can be dropped and the core-side detection reads the native chunk. |
| 12 | `e322992fe0` | feat(genode): sel4 kernel: early framebuffer text console for UEFI/no-serial boot visibility | `genode/contrib/sel4-*/src/kernel/sel4/src/arch/x86/kernel/fb_console.c` (new) + `include/arch/x86/arch/kernel/fb_console.h` (new) + `src/plat/pc99/machine/io.c` (`kernel_putDebugChar` mirrors every character to the FB after the UART write) + `src/arch/x86/config.cmake` (`kernel/fb_console.c` added to the x86 CFILES list) + `src/arch/x86/kernel/boot_sys.c` (`fb_console_init()` invoked right after the multiboot2 FB tag is parsed; `fb_console_enter_kernel_window()` invoked right after `setCurrentVSpaceRoot()`), via `docs/patches/sel4-early-fb-console.patch`. The console captures `boot_state.fb_info` and renders a 8×8 ASCII glyph (95-glyph embedded font, ~760 B .rodata) for every byte that flows through `kernel_putDebugChar` — i.e. BOTH the kernel's `printf` path (via `putchar`) AND the `SysDebugPutChar` syscall path that carries every line of Genode core's log. Two-phase init matches the kernel's two paging modes: pre-`setCurrentVSpaceRoot` paging is on with the boot_pml4 (identity map of the first 4 GiB) so the FB is reached via its physical address; post-`setCurrentVSpaceRoot` the cached VA is rewritten via `paddr_to_pptr()`. No-op (and disabled) when `boot_state.fb_info.addr == 0`, `bpp != 32`, `type != 1`, or `phys >= PADDR_TOP` (512 GiB) — so a normal BIOS boot's UART-only log is unchanged. The console clears the screen to black on init and emits a `sel4 fb console: ready\n` banner so the operator can confirm the patch is alive on real hardware before any kernel_putDebugChar fires. | Phase 15 W3 problem 15-3 (real-hardware visibility): the Sponge OS boot chain on the LG gram 17ZD90N has NO serial port (UEFI-only laptop); bender's 0xB8000 VGA-text writes are invisible under UEFI GOP; the seL4 kernel log is COM1-only; Genode core's log reaches the kernel via `SysDebugPutChar` → `kernel_putchar` → `kernel_putDebugChar`. After GRUB2's splash, every diagnostic byte was structurally blind — the user could not see whether core's `Untyped_Retype` was the issue, a missing ROM, a `cap` exhaustion, or whether the boot reached Genode at all. Mirroring every `kernel_putDebugChar` to the GOP framebuffer gives the user a single, end-to-end diagnostic surface that survives into userland (SysDebugPutChar). Verified: `make -C genode/build/x86_64 run/sponge-desktop-disk-uefi KERNEL=sel4 BOARD=pc` boots on QEMU/OVMF (-vnc :N + monitor `screendump`), the kernel emits `sel4 fb console: ready` followed by `Booting all finished, dropped to user space` on the GOP framebuffer, then `Genode …` and the init tree render via boot_fb's Capture session; BIOS boot (`run/sponge-minimal`) is unaffected because `boot_state.fb_info.addr` is zero and the console stays disabled. The console is intentionally minimal (760 B font, one putchar path) — large enough to read log lines, small enough to not warrant a vt100 emulator. NOTE: `genode/contrib/` is not git-tracked; the patch file is the durable record, re-applied on `prepare_port sel4`. | Sponge OS acquires a graphics-capable vendor that ships serial-on-PCI, or the GOP framebuffer becomes the canonical kernel log channel everywhere and upstream seL4 mirrors the console to the framebuffer in its debug builds. **W4 (Phase 15) deferral update — regenerated:** the on-disk fb_console.c/h were modified to defer all drawing (clear + banner) AND every fb_console_putchar() byte until *after* setCurrentVSpaceRoot() whenever the GOP framebuffer `phys` is >= 4 GiB (4 * 2^30). On x86_64 the boot_pml4 identity-maps only the first 4 GiB via 2 MiB pages, so writing to a >= 4 GiB physical address from the identity-mapped kernel window faults the kernel inside the first memset() with #PF W:1 P:1 (the W3-eager-paint path). The new `phys_high` flag is set in `fb_console_init()`; the clear + banner are factored into `fbc_paint_clear_and_banner()` and called from `fb_console_enter_kernel_window()` after `paddr_to_pptr()` is valid (kernel PML4 covers phys < PADDR_TOP = 2^39 on x86_64). `fb_console_putchar()` no-ops on the deferred-init gate so the identity-mapped VA is never dereferenced for a high FB. All four W3 guards (no-multiboot2-FB-tag / bpp!=32 / type!=1 / phys>=PADDR_TOP) are preserved verbatim; the only behavioral change is the deferred-init gate for the >= 4 GiB boundary (4 * 2^30, defined as FBC_HIGH_PADDR_THRESHOLD = 0x100000000ULL). Patch file: `docs/patches/sel4-early-fb-console.patch` (regenerated, ~1530 lines, 5 file hunks: header + fb_console.c new + config.cmake +1 line + boot_sys.c +2 calls + io.c +1 line). The new FBC_HIGH_PADDR_THRESHOLD constant and the resulting deferred_init_pending / phys_high state fields are documented in fb_console.h at the top of the file header. **W5 readability update:** the glyph cell is now 16x16 px (`FBC_SCALE = 2` — each 8x8 font bit paints a 2x2 pixel block) because the 8x8 cell was unreadable on the 17ZD90N's 2560x1600 panel in the v11 round (the boot log appeared but the operator could not read which component aborted). Patch regenerated and round-trip verified (pristine state -> `git apply` == working tree). **W6 kill-switch update (post-desktop-visible):** with the DE live on the panel, the console's log mirror kept scrolling text over the desktop (boot_fb's Capture blit repaints only nitpicker's dirty regions, so console text outside them persisted — "the log pushes the DE off the screen"). The patch now supports the exact kernel-cmdline token `fbcon=off` (new `fb_console_check_disable(const char *cmdline)`, called from the MULTIBOOT2_TAG_CMDLINE branch of `try_boot_sys_mbi2()`; a `fbc_disabled_by_cmdline` flag outside the memset-ed `fbc` struct keeps the disable stable under any CMDLINE/FB tag order): no clear, no banner, no character painting, while the serial log stays untouched, and a one-line serial notice (`sel4 fb console: disabled by cmdline (fbcon=off), serial log unaffected`) records the event. The PRODUCTION media passes the token on GRUB's `module2 /boot/sel4 … fbcon=off` line (`run/sponge-desktop-disk-uefi-usb.run`); the diagnostic/test profile omits it, keeping the on-panel boot log for bring-up (control escape hatch per AGENTS §1.1). Patch regenerated (5 file hunks incl. the new call site) and round-trip verified against pristine `cd6d3b8c25d4` (simulated contrib root: `git apply` == working tree, all 5 files byte-identical). |

| 13 | `c310e72dc7` | feat(genode): sel4 kernel: bound device-untyped coverage above the memory map (high GOP FB + 64-bit PCI BARs) | `genode/contrib/sel4-*/include/kernel/boot.h` adds `paddr_t highest_map_addr;` to `ndks_boot_t`; `genode/contrib/sel4-*/src/kernel/sel4/src/kernel/boot.c` `init_freemem()` captures `highest_map_addr = max(available[i].end)` from the multiboot2 RAM map, and `create_untypeds()`'s trailing region (previously `[start, CONFIG_PADDR_USER_DEVICE_TOP)`) is replaced by a LOW + HIGH split: `LOW = [ROUND_UP(start, 31), ROUND_UP(highest_map_addr, 21))` (2-GiB-aligned start keeps per-untyped size at 2 GiB instead of 32 MiB, so 1-4 untypeds cover the LAPIC/IO-APIC/HPET block at ~4 GiB); `HIGH = [ROUND_UP(highest_map_addr, 21), SPONGE_DEVICE_UNTYPED_TOP=2^40)` (1 TiB ceiling; device untypeds sized at the start alignment — 8 GiB on 8 GiB systems, 4 GiB on 4 GiB systems — so 1-127 untypeds cover every practical GOP framebuffer above the map, including the LG gram 17ZD90N Insyde H2O GOP at ~0x800000000 and 64-bit PCI BARs in the q35 pci-hole64). `SPONGE_DEVICE_UNTYPED_TOP` is a new `#define` in `boot.c` (above the `ndks_boot` BSS decl). Total device-untypeds on a typical 8 GiB system: 3 (LOW) + 7 (HIGH, ceiling=2^36...2^40) + 1 (RAM) = ~11, well within `CONFIG_MAX_NUM_BOOTINFO_UNTYPED_CAPS=160`; the trailing-region cap-stop gap on 4 GiB systems (254 LOW + 15 HIGH + 1 RAM = ~270) is intentionally NOT raised because `seL4_BootInfoFrameBits == seL4_PageBits == 12` constrains the bootinfo frame to 4 KiB (any cap > ~246 would trigger `invalid_seL4_BootInfoFrameSize`). The `start >= high_map_end` degenerate branch (large kernel image) falls back to the upstream single trailing region verbatim. The HIGH range never reclassifies map-described RAM — `ROUND_UP(highest_map_addr, 21)` is the start, so every padder below it remains classified as RAM (not device). Patch file: `docs/patches/sel4-device-untyped-coverage.patch` (W4 commit, ~250 lines, two file hunks: boot.h +1 field + boot.c +~140 lines). The Sponge-side `kernel-sel4-pc.mk` bootinfo cap (160) and toolchain-sed overrides are unchanged — the fix is localized to the contrib/ tree only. | Phase 15 W4 problem 15-3: the GOP framebuffer on the LG gram 17ZD90N's Insyde H2O firmware sits above the memory map (at ~0x800000000 = 32 GiB, alongside the xHCI BAR also at 0x800000000). Without the row-13 patch, the kernel's device-untyped coverage stops at `ctzl(start) = 25` alignment (32 MiB per untyped) where `start` is the kernel+modules end (typically 0x9000000 on x86_64). The bootinfo cap (160 untypeds) fills up after ~5 GiB of coverage — never reaching the high GOP FB address — so Genode core's platform driver reports `I/O memory [0000000800000000,...) not available` and `boot_fb` exits. Same failure class observed for xHCI at 0x800000000. After the patch: the LOW range uses `ctzl(ROUND_UP(start, 31)) = 31` alignment (2 GiB per untyped), the HIGH range uses `ctzl(ROUND_UP(highest_map, 21))` alignment at the start of the post-map region. The GOP FB untyped is unblocked; the desktop chain reaches `alpha-probe: PASS` instead of black-screening. The TODO-19 honest-register row continues to track the real-hardware diagnostic. | Upstream seL4 grows `seL4_BootInfoFrameBits` to enable larger bootinfo frame sizes (which would let the cap rise past ~246 untypeds); or Sponge OS moves to a kernel with a `get_high_mmio_untyped_cap(phys, size)` API that lets Genode core request device-untypeds by address range instead of receiving one giant pre-populated bootinfo frame. |
| 14 | `69db536604` | feat(genode): base-sel4 high-phys frame-cap CNode with sequential slot allocation (IO_MEM at phys >= 8 GiB) | `genode/repos/base-sel4/src/core/`: `include/core_cspace.h` adds `NUM_HIGH_PHYS_SEL_LOG2 = 14` (2^14 slots, 512 KiB backing), `HIGH_PHYS_BASE = 8 GiB` (the low flat CNode ceiling), `high_phys_cnode_sel()`, `TOP_CNODE_HIGH_PHYS_IDX = 0x7e0`, and the `high_phys_frame_phys[]` slot table with `high_phys_slot_alloc/find/free` helpers; `include/platform.h` constructs `_high_phys_cnode` as a direct member from the initial untyped pool (early, like the other static CNodes); `platform.cc` registers high-phys device untypeds in `_io_mem_alloc` without eager retype (whole untyped on the first batch — per-batch registration would explode the AVL tree), wires the cap into the top CNode (slot 0x7e0) AND the core CNode (its flat selector slot) inside `_switch_to_core_cspace` — both wirings are load-bearing, as is doing them pre-switch; `include/untyped_memory.h` has the slot-based `_high_phys_sel`, table-first `frame_sel`, high-phys branches in `convert_to/from_page_frames`, and the `_high_phys_untyped_sel` bootinfo scan; `include/vm_space.h` + `spec/x86/vm_space.cc` have the table-first `frame_source_cnode`/`frame_source_index` (misses fall back to the v1 formula — REQUIRED: ROM/RAM dataspace "phys" values are phys-CNode-slot-encoded and can exceed 8 GiB); `platform_pd.cc` reserves top slot 0x7e0; `io_mem_session_support.cc` keeps the entry-point check. | Row 13 makes the kernel publish device untypeds above the map, but core's flat phys CNode (2^21 slots x 4 KiB = 8 GiB) could not store their frame caps. The v1 flat-window design (2^23 slots indexed by (phys - 8 GiB) >> 12) failed on the 17ZD90N twice: its 256 MiB contiguous backing cannot be allocated on the Insyde-fragmented memory map, and the real targets — GOP FB at 0x4000000000 (256 GiB), PCH xHCI BAR at 0x601d140000 (~384.5 GiB) — sit 128 GiB apart, so no affordable flat window covers both. Sequential slots + a phys->slot table remove both the backing size and any address-window assumption. Verification also caught v1 latent defects: the lazy creation path breaks (initial-thread-CNode slot resolution fails after the cspace switch), top slot 0x7e0 was unreserved (clobberable by vm_space leaf CNodes), and the core-CNode wiring was missing (runtime selector resolution). Verified: `run/sponge-desktop-disk-uefi-usb` with `nec-usb-xhci` (BAR at 32 GiB) reaches `sponge_configd: ready` (xhci_hcd binds, USB 3.0 enumerates); regressions `sponge-minimal`, `sponge-fbprobe-uefi`, `sponge-desktop-disk` (alpha-probe PASS) all green. | Upstream seL4/Genode grows a proper high-MMIO IO_MEM path (large-page device frames or a windowed phys CNode scheme). |
| 15 | `f60c3aedc3` | fix(genode): sel4: serve 16K-pool AVL metadata from a static arena (same-thread re-entrancy deadlock) | `genode/repos/base-sel4/src/core/spec/x86/platform_thread.cc`: `phys_alloc_16k()` grows its `Allocator_avl_tpl` slab against `Avl_metadata_arena` — a function-local-static, 4096-aligned, 4 MiB bump `Genode::Allocator` (`try_alloc` bumps a cursor, `free`/`_free` are no-ops, exhaustion returns `OUT_OF_RAM`) — instead of core's `Core_mem_allocator`. The arena MUST be a function-local static: file-scope object constructors do not run before first use in core, leaving a null vtable (observed as a rootserver vm fault at vptr+0x10 inside `Slab::_new_slab_block` on the first growth). | Growing the pool's AVL tree routes slab-block allocation through its md_alloc: `Core_mem_allocator::alloc_aligned` (takes the core-mapping mutex) → `Mapped_mem_allocator::_map_local` → page tables / vm-leaf CNodes — which allocate 16-KiB pages from the very pool being grown, re-entering `phys_alloc_16k().alloc_aligned` while `Synced_range_allocator` already holds the pool's mutex in the outer frame. Same-thread mutex re-acquire → `Error: deadlock ahead` with the return ip in `synced_range_allocator.h:114` (owner pinned by .bss symbol math on the unstripped debug core: mutex = pool object +0x410). Latent upstream; triggered by Phase-15 runtime concurrency (boot_fb's 20 ms blit timer × the USB storage chain — the row-14 high-phys CNode alone keeps ~16 K live ranges, so the tree grows well past its initial blocks). Sizing: ~85 nodes per 4 KiB slab block → ~87 K nodes in 4 MiB. Verified: `run/sponge-desktop-disk-uefi-usb` (blit timer enabled) passes the full storage-chain gate with deadlock count 0 and `configd: ready`; regressions `sponge-minimal`, `sponge-fbprobe-uefi`, `sponge-desktop-disk` (alpha-probe PASS) green. An alloc/free range-churn "pre-grow" variant was tried first and does NOT work (boot-time free nodes absorb the churn; runtime demand is unbounded). | Upstream rewires `phys_alloc_16k` metadata growth so it cannot re-enter the guarded pool (verified absent as of 2026-08-24: Codeberg staging `492a510242..8629d0e91d` = 223 commits, none touching the allocator files or matching deadlock/re-entrancy keywords; upstream's own precedent `ff997e46d79` REPLACED an AVL-backed allocator with dictionaries for the same metadata-growth reason). |
| 16 | `b3b070ce3f` | fix(genode): packet_stream tx_wakeup re-checks queue state (self-healing wakeup) | `genode/repos/os/include/os/packet_stream.h` `Packet_descriptor_transmitter::tx_wakeup()`: submit the wakeup signal when `_tx_wakeup_needed` OR the tx queue is non-empty (read under the same `_tx_queue_mutex` that guards `try_tx`, so the producer's own enqueue is always visible to the check). The rx direction is untouched. | Upstream issue **genodelabs/genode#5338** (open since 2024-09-02, still unfixed through release 26.08 — verified 2026-08-26 on Codeberg staging `492a510242..d18ee9e9`: packet_stream.h byte-identical, zero commits in range, nothing to backport): the empty→nonempty transition flag can fail to arm when producer and consumer observe different queue states (`single_element()` wrongly false), permanently suppressing the wakeup while packets wait — the consumer sleeps forever. Surfaced on the 17ZD90N as the permanent USB-mouse input stall (exactly the enumeration-time HID reports delivered, then silence, while the usb host driver kept receiving completions — host-sig climbing, usb-sig frozen; replug left usb_hid asleep), and reproduced transiently in QEMU as a ~35 s stall window with spontaneous recovery via an unrelated signal (the lost-wakeup signature). The fix is ordering-agnostic and self-healing: a stale flag suppresses at most one wakeup; every subsequent completion re-signals because packets remain queued. Worst case is a redundant signal, which the receiver coalesces. Verified: 3-phase QEMU soak at 2560x1600 (immediate post-probe burst — the historical stall window — plus bursts after 20 s/45 s idle) shows uninterrupted flow (usb-sig 150 == evdev-batch 150 == np ev 151); regressions `sponge-desktop-disk-uefi-usb` (usb_block storage chain exercises the same packet stream), `sponge-minimal` green. | Upstream fixes #5338 (release-store/acquire-load on the queue indices or an equivalent robust-wakeup protocol); drop this row and re-verify the mouse soak. |
...
| bender `check_mem()` EFI-type filter | `genode/tool/boot/bender` (binary; rebuild from `alex-ab/morbo` branch `genode_bender` per `genode/tool/boot/README`) | Under OVMF UEFI boot, bender rejects the seL4 kernel placement: `Reserved memory 800000+8000 type=4 overlaps with phdr 20835f+609901` — type 4 (EfiBootServicesData) is reclaimed after ExitBootServices, so the rejection is over-conservative. A principled rebuild would tolerate EFI types {1,2,4,5,7,14} and keep rejecting {0,6,8,9,10,11,12,13}. A temporary 5-byte NOP at file offset 0x2e0b was tested in Phase 15 W1 and **reverted** (over-broad; core still hung for a different reason). Evidence: `docs/evidence/phase15-uefi-boot-smoke.log`. | 15+ |
| g2fg GRUB2 rebuild (`grub213` branch or newer) | `genode/contrib/grub2-*/boot/grub2/grub2_64.efi` (prebuilt; port `grub2.port` fetches `alex-ab/g2fg.git`) | The vendored prebuilt (2025-03, grub 2.12) deterministically page-faults (#PF W:1 P:1) in its multiboot2 load path under edk2 ≥ 2025.02 OVMF builds (the edk2 W^X regression: tianocore/edk2 #11046/#11074, RH #2354064, Debian #1099500). g2fg has a `grub213` branch that may carry the upstream relocator fixes. Currently mitigated for QEMU by the pinned OVMF 2024.02 in `var/ovmf/` (gitignored, SHA-256 recorded in `run/sponge-boot-uefi.run`). | 15+ |
...
| `coreutils` | `c910bf8c769e6d35cbf803f24e1ecae709aff392` | `https://ftpmirror.gnu.org/coreutils/coreutils-8.29.tar.xz` (GPLv3) | GNU coreutils 8.29, built as `noux-pkg/coreutils-minimal` (`coreutils-minimal.tar`: cat/cp/ls/mkdir/mv/rm/rmdir/sort/tail/md5sum) — the Phase 13 CLI toolset inside `pkg/terminal` (docs/plans/phase13-package-ecosystem.md D13.2). Its gnulib build regenerates `lib/iconv_open-*.h` with `gperf` — see the `gperf` host-tool requirement in §7. |
| `grep` | `a08c4f7925aa5af491cb243e3e37cf5dd8bb9c06` | `https://ftpmirror.gnu.org/grep/grep-3.1.tar.xz` (GPLv3) | GNU grep 3.1, built as `noux-pkg/grep` (`grep.tar`). Links `pcre`. Phase 13 CLI toolset. |
| `sed` | `08ac8610ea1a59e7562c66800acecc55a47e93b3` | `https://ftpmirror.gnu.org/sed/sed-4.4.tar.xz` (GPLv3) | GNU sed 4.4, built as `noux-pkg/sed` (`sed.tar`). Links `pcre`. Phase 13 CLI toolset. |
| `tar` | `11ef1926fb0a9097dd097d4672ad8567c7ec8c96` | `https://ftpmirror.gnu.org/tar/tar-1.30.tar.xz` (GPLv3) | GNU tar 1.30, built as `noux-pkg/tar` (`tar.tar`). Phase 13 CLI toolset. |
| `less` | `609181c8cfc59adfaa6a8e94bf3622df1a7eb3f0` | `https://ftpmirror.gnu.org/less/less-487.tar.gz` (GPLv3) | less 487 pager, built as `noux-pkg/less` (`less.tar`). Links `ncurses`. Phase 13 CLI toolset. |
| `findutils` | `48d3049df9b99b9672e36a1b0f15f425e37af65c` | `https://ftpmirror.gnu.org/findutils/findutils-4.6.0.tar.gz` (GPLv3) | findutils 4.6.0 (find/xargs), built as `noux-pkg/findutils` (`findutils.tar`). Phase 13 CLI toolset. |
| `diffutils` | `f2f34ac57062c071d127b8fc40a051278f2f3aef` | `https://ftpmirror.gnu.org/diffutils/diffutils-3.3.tar.xz` (GPLv3) | diffutils 3.3 (diff/cmp/diff3/sdiff), built as `noux-pkg/diffutils` (`diffutils.tar`). Phase 13 CLI toolset. |
| `which` | `1b179015279594d0dd81beb5831bbb2d893221b9` | `https://ftpmirror.gnu.org/which/which-2.21.tar.gz` (GPLv3) | which 2.21, built as `noux-pkg/which` (`which.tar`). Phase 13 CLI toolset. |
| `mupdf` | `5b492514a8e9ba4db6bbf54dee54b94f068f0dae` | `http://mupdf.com/downloads/archive/mupdf-0.9-source.tar.gz` (GPLv2) | MuPDF 0.9 PDF rendering library, linked by `app/pdf_view` (`pkg/pdf_view`, Phase 13 W4). Lives in the `libports` repo. |
| `openjpeg` | `721d101479429b2435349579ba5f2bdea84430b9` | `https://github.com/uclouvain/openjpeg/archive/version.1.4.tar.gz` (BSD) | OpenJPEG JPEG-2000 codec, a mupdf image-decoder dependency. |
| `jbig2dec` | `68226914536c0fdb1fb42fd87685989a43a8ea92` | `https://github.com/ArtifexSoftware/ghostpdl-downloads/releases/download/gs924/jbig2dec-0.15.tar.gz` (GPLv3) | JBIG2 decoder, a mupdf image-decoder dependency. |
| `pcre` | `11003e42ac571da33b5e657a6500bc2b93884a8a` | `http://sourceforge.net/projects/pcre/files/pcre/8.30/pcre-8.30.tar.bz2` (BSD) | PCRE regex library, linked by `noux-pkg/grep` and `noux-pkg/sed`. |
| `qt6_tools` | `cdc19bb65058adf68bc97433df640b996e312621` | `https://download.qt.io/archive/qt/6.8/6.8.3/submodules/qttools-everywhere-src-6.8.3.tar.xz` (GPLv3) | Qt6 tools module — only its `examples/designer/calculatorform` sources are used, mirrored by `recipes/src/qt6_calculatorform` and built as `app/qt6/examples/calculatorform` for `pkg/calculator` (Phase 13 W3). |
