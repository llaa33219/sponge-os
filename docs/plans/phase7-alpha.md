...
- Evidence: .omo/evidence/task-<N>-phase7-alpha.<ext>
...
- [x] 2. Doc-first: amend docs/06-vct.md (new subcommands + drift fix)
  ...
  QA scenarios: happy — amended doc reviewed for §3.3 compliance of every new entry (automation default + escape hatch + --json + help); failure — confirm no §4 entry promises interactive [Y/n] prompts (forbidden by Scope OUT). Evidence .omo/evidence/task-2-phase7-alpha.md
...
- [x] 3. seL4 media smoke test (minimal ISO + disk boot)
  ...
  QA scenarios: happy — both media reach `[init -> vct] vct` banner within the run-tool timeout; failure — remove one staged boot module (e.g. ld.lib.so) in a scratch copy and confirm the run FAILS loudly by timeout (validates the gate isn't vacuous). Evidence .omo/evidence/task-3-phase7-alpha.log
...
- [x] 4. Unified desktop scenario `run/sponge-alpha.run`
  ...
  QA scenarios: happy — PASS marker appears after all four sub-assertions; failure — deliberately drop the configd start node in a scratch copy and confirm the probe FAILS (not hangs) — validates fail-loud gating. Evidence .omo/evidence/task-4-phase7-alpha.log
...
- [x] 5. Disk-image media of the alpha desktop + QEMU boot QA
  ...
  QA scenarios: happy — media boot == direct boot markers; failure — corrupt one byte in a scratch copy of the .img and confirm the boot does NOT produce PASS (media is actually being used). Evidence .omo/evidence/task-5-phase7-alpha.log
...
- [x] 6. ISO media of the alpha desktop + QEMU boot QA
  ...
  QA scenarios: happy — PASS from CD boot; failure — boot the .iso with the probe's PASS string altered in a scratch build and confirm timeout failure. Evidence .omo/evidence/task-6-phase7-alpha.log
...
- [x] 7. tool/dist (Mojo) + docs/08 manual steps + docs/11 host-tool rows
  ...
  QA scenarios: happy — full run green on a clean var/dist; failure — temporarily rename `xorriso` in PATH and confirm tool/dist prints the apt line and exits non-zero BEFORE invoking any build. Evidence .omo/evidence/task-7-phase7-alpha.log
...
- [x] 9. Installed-vs-running lifecycle model in sponge_pkgd
  ...
  QA scenarios: happy — install→launch→running transitions asserted; failure — `launch` on a not-installed package returns the `not-installed` result (asserted), and double-launch returns `already-running`. Evidence .omo/evidence/task-9-phase7-alpha.log
...
- [x] 10. Click-to-launch in Sponge DE launcher + `vct launch`
  ...
  QA scenarios: happy — both launch paths verified; failure — `vct launch` on an unknown package prints the not-installed error and exits non-zero (asserted in scenario log). Evidence .omo/evidence/task-10-phase7-alpha.log
...
- [x] 11. Depot-import spike + tool/pkg_import (host-side repackaging)
  ...
  QA scenarios: happy — both imports produce pkg dirs; failure — run tool/pkg_import with a bogus depot ref and confirm a clear non-zero error, no partial pkg/ directory left behind. Evidence .omo/evidence/task-11-phase7-alpha.log
...
- [x] 12. Networking spike: `run/sponge-net-probe.run` on base-sel4
  ...
  QA scenarios: happy — content round-trip; failure — stop the host fixture and confirm the run fails by bounded timeout (no silent hang), proving the gate. Evidence .omo/evidence/task-12-phase7-alpha.log
...
- [x] 13. Terminal package (`pkg/terminal`) — gems terminal + bash/vim
  ...
  QA scenarios: happy — full path; failure — remove the font ROM in a scratch staging and confirm bounded-timeout failure with an identifiable log (font missing), not a hang. Evidence .omo/evidence/task-13-phase7-alpha.log
...
- [x] 14. Text editor package (`pkg/textedit`)
  ...
  References: todo 11 (import tooling + pin); run/sponge-launcher.run:126-136 (Qt app config shape); todo 8 (config/metadata support); .omo/drafts/phase7-alpha.md A8.
  ...
  QA scenarios: happy — window appears; failure — textedit binary missing from staging → pkgd install reports a clear missing-binary error (asserted), not a crash. Evidence .omo/evidence/task-14-phase7-alpha.log
...
- [x] 15. `sponge_files` — Sponge-native minimal Qt6 file manager
  ...
  QA scenarios: happy — four assertions pass; failure — delete in the read-only area is refused and the refusal is surfaced in the UI/log (asserted), not silently ignored. Evidence .omo/evidence/task-15-phase7-alpha.log
...
- [ ] 17. vct shutdown/reboot via platform System session
  ...
  QA scenarios: happy — both commands observed; failure — `vct shutdown --json` on a scenario without the System route prints a clear "service unavailable" error, exits non-zero (asserted), and the guest keeps running. Evidence .omo/evidence/task-17-phase7-alpha.log
  Commit: Y | feat(vct): shutdown and reboot via platform System session
- [ ] 18. vct update + vct search
...
- [ ] 20. Full regression + release verification
  ...
  QA scenarios: happy — suite green end to end; failure — any non-zero scenario blocks the release; the failing log + diagnosis goes into the evidence index and the todo that owns it is reopened. Evidence .omo/evidence/task-20-phase7-alpha.md
...
Plus: zero regressions across the pre-existing 20-scenario suite (todo 20), and every todo's evidence file present in .omo/evidence/.
