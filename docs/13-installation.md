...
- **Cross-component clipboard is delivered (Phase 14 W5).** The clipboard service reuses the upstream Genode `os/src/server/clipboard` per decision D14.2; the cross-component copy/paste proof (copy in one separately-launched component, paste in another, per U2) is delivered by W5. `run/sponge-clipboard.run` proves the cross-component bus write (sentinel byte-for-byte in the server-side ROM); `run/sponge-clipboard-focus.run` proves the focus-aware write gating (`match_labels="yes"` sub-scenario). The visual paste is best-effort — the structural byte-match in the bus ROM is the canonical proof.
