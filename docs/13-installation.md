...
## 4. Write the disk image to USB (real-hardware boot)

**Real-hardware boot is verified** (Phase 15 15-3, 2026-08-23): the desktop `.img` boots the LG gram 17ZD90N-VX7BK from USB to the same compositor-verified desktop state as QEMU — the on-panel boot log ends with `child "alpha_probe" exited with exit value 0` (themed-panel pixel check, launcher feed, and configd broadcast all passed through the real compositor). The full machine preparation (BIOS settings), boot procedure, evidence interpretation, and pre-registered gaps (trackpad dead — use a USB mouse; Wi-Fi unsupported) are documented in the binding protocol: [`docs/plans/phase15-hardware-boot-protocol.md`](plans/phase15-hardware-boot-protocol.md). The target machine is recorded in [`docs/15-hardware-compatibility.md`](15-hardware-compatibility.md) row 17. Boot media is firmware-specific: the same `tool/dist` build with `--firmware uefi` (as below) for the 17ZD90N; the Phase-12 BIOS-side USB verification (`run/sponge-usb-boot.run`) covers SeaBIOS/QEMU only.
...
- **System tray and Advanced-Mode panel menu → Phase 15+.** The Phase 14 panel is intentionally minimal (clock + launcher only); the system tray and the Advanced-Mode panel menu entry that opens Leitzentrale are deferred to Phase 15+ per decision D14.7 in `docs/plans/phase14-daily-desktop.md`. In this release, the only way to open Leitzentrale is the `vct leitzentrale` CLI command.
- **Cross-component clipboard is delivered (Phase 14 W5).** The clipboard service reuses the upstream Genode `os/src/server/clipboard` per decision D14.2; the cross-component copy/paste proof (copy in one separately-launched component, paste in another, per U2) is delivered by W5. `run/sponge-clipboard.run` proves the cross-component bus write (sentinel byte-for-byte in the server-side ROM); `run/sponge-clipboard-qtsettext.run` proves the Qt→server write path is healthy (`QGenodeClipboard::setMimeData` round-trips through the server, programmatic `setText()` harness); `run/sponge-clipboard-focus.run` proves the focus-aware write gating (`match_labels="yes"` sub-scenario). Known limitation: keyboard Ctrl-C copy inside the `qt6_textedit` example does not fire on base-sel4 (investigated in `docs/evidence/phase14-w5-qtwrite-failure.md`; the bridge itself is proven healthy, the failure is downstream in the shortcut/input layer — a Phase 15+ item). The visual paste is best-effort — the structural byte-match in the bus ROM is the canonical proof.
- **Alpha media pre-stages the five Phase-13 packages.** Per decision D14.10, the Alpha media now pre-stages `terminal`, `textedit`, `files`, `calculator`, and `pdf_view` (all five boot-verified from Phases 7–13). Launcher category mapping: terminal → `System`, textedit → `Office`, files → `System`, calculator → `Utilities`, pdf_view → `Office`. Closes the Phase 13 carryover "calculator / pdf_view not wired into Alpha media". Falkon remains opt-in via D14.5.
