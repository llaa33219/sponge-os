...
- **Real-hardware boot → Phase 15 (in progress, display gap).** The UEFI boot chain is proven end-to-end on QEMU/OVMF (`run/sponge-desktop-disk-uefi.run` reaches `alpha-probe: PASS` — the seL4 kernel `init_freemem` UEFI fix + principled bender + full GRUB; see `docs/11` patch-ledger rows 10–11). On the physical LG gram 17ZD90N the chain reaches GRUB, but GRUB cannot produce a working payload framebuffer on the real iGPU GOP (the fbprobe2 GOP-draw probe freezes at `no console will be available to OS`; the panel offers valid 32bpp modes per `videoinfo`). So the desktop does not render on the real machine yet — the open item is the GRUB-video-driver-on-real-GOP path, not Sponge OS or seL4. The single `target: real-hardware` row in `docs/15-hardware-compatibility.md` stays a `gap` until the display path works. Full bring-up log: `docs/evidence/phase15-index.md` §13–§14.
