...
- **Cross-component clipboard is in progress (Phase 14 W5).** The clipboard service reuses the upstream Genode `os/src/server/clipboard` per decision D14.2; the cross-component copy/paste proof (copy in one separately-launched component, paste in another, per U2) is delivered by W5. Until W5 lands, only intra-process Qt clipboard is available.
