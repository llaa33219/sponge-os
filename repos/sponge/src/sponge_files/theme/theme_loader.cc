 ...
 * background, title text, accent, list alternating row, error/success/
 * warning, default font, padding). Unknown keys are tolerated with a
 * warning so any valid Sponge DE theme file is accepted unchanged.
 *
 * Depends only on Genode base headers — no std::string, no heap
 * allocation, no exceptions (AGENTS.md §1.4 — no empty catch and no
 * type bypass).
 */
...
using namespace Sponge::Sponge_Files::Theme;
...
bool ThemeLoader::load(char const *data, Genode::size_t len, Theme &out)
{
	...
	/* ROM dataspaces are page-aligned; trim trailing NUL padding. */
                              ...
                              Genode::size_t line_no)
{
	...
	if (line[start] == '[' && line[start + trimmed_len - 1] == ']') {
		...
		if (sec_len == 0 || sec_len >= 32) {
			Genode::warning("files: theme parse error line ", line_no,
                             ...
                             Genode::size_t line_no)
{
	...
	if (Genode::strcmp(section, "meta") == 0) {
		if (Genode::strcmp(key_buf, "version") == 0) {
			...
			if (v != 1) {
				Genode::warning("files: theme unsupported version ", v,
		...
		if      (Genode::strcmp(key_buf, "window_bg")     == 0) target = &_theme->_window_bg;
		...
		else if (Genode::strcmp(key_buf, "list_alt")      == 0) target = &_theme->_list_alt;
	...
	/* Unknown keys (panel_bg, launcher_width, ...) are tolerated — they
	 * belong to sponge-de and are simply not consumed here. */
	return true;
}
