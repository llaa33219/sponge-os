...
static int vmmr0_pdm_queue_create(PVMR0 pvmr0, PDMQUEUECREATEREQ &request)
{
	Sup::Vm &vm = *(Sup::Vm *)pvmr0;

	uint32_t iQueue = vm.pdm.s.cRing0Queues;
	if (iQueue >= RT_ELEMENTS(vm.pdm.s.apRing0Queues))
		return VERR_OUT_OF_RESOURCES;

	uint32_t const cItems   = request.cItems;
	/* keep bitmap in it's own cacheline  */
	uint32_t const cbBitmap = RT_ALIGN_32(RT_ALIGN_32(cItems, 64) / 8, 64);
	uint32_t const cbQueue  = RT_OFFSETOF(PDMQUEUE, bmAlloc)
	                        + cbBitmap
	                        + uint32_t(request.cbItem) * cItems;

	PPDMQUEUE pQueue = (PPDMQUEUE)RTMemPageAllocZ(cbQueue);
	if (!pQueue) return VERR_NO_PAGE_MEMORY;

	pdmQueueInit(pQueue, cbBitmap, uint32_t(request.cbItem), cItems,
	             request.szName, PDMQUEUETYPE(request.enmType),
	             request.pfnCallback, request.pvOwner);

	vm.pdm.s.apRing0Queues[iQueue] = pQueue;
	request.hQueue                 = iQueue;

	vm.pdm.s.cRing0Queues++;

	return VINF_SUCCESS;
}


static int vmmr0_pgm_phys_allocate_ram_range(PVMR0 pvmr0, PGMPHYSALLOCATERAMRANGEREQ &request)
{
	Sup::Vm *vm = (Sup::Vm *)pvmr0;

	return pgmPhysRamRangeAllocCommon(vm, request.cGuestPages, request.fFlags, &request.idNewRange);
}
...
static int vmmr0_pgm_phys_mmio2_register(PVMR0 pvmr0, PGMPHYSMMIO2REGISTERREQ &request)
{
	Sup::Vm *vm = (Sup::Vm *)pvmr0;

	if (request.fFlags & PGMPHYS_MMIO2_FLAGS_USE_EXISTING_BACKING)
		return pgmPhysMmio2RegisterWorkerExisting(vm, request.cGuestPages, request.idMmio2,
		                                              request.cChunks,     request.pDevIns,
		                                              request.iSubDev,     request.iRegion,
		                                              request.fFlags,      request.pbR3);

	return pgmPhysMmio2RegisterWorkerAlloc(vm, request.cGuestPages, request.idMmio2,
	                                           request.cChunks,     request.pDevIns,
	                                           request.iSubDev,     request.iRegion,
	                                           request.fFlags);
}
...
static int vmmr0_pgm_phys_rom_allocate_range(PVMR0 pvmr0, PGMPHYSROMALLOCATERANGEREQ &request)
{
	Sup::Vm *vm = (Sup::Vm *)pvmr0;

	return  pgmPhysRomRangeAllocCommon(vm, request.cGuestPages, request.idRomRange,
	                                       request.fFlags);
}


/* same as in tmR3TimerQueueGrow (TMR3.cpp) */
static int vmmr0_tm_grow_timer_queue(PVMR0 pvmr0, uint32_t idxQueue, uint32_t cNewTimers)
{
	Sup::Vm &vm = *(Sup::Vm *)pvmr0;

	AssertReturn(cNewTimers <= _32K && idxQueue < TMCLOCK_MAX, VERR_TM_TOO_MANY_TIMERS);
	ASMCompilerBarrier();

	if (idxQueue >= TMCLOCK_MAX) return VERR_TM_INVALID_TIMER_QUEUE;

	PTMTIMERQUEUE  pQueue       = vm.tm.s.aTimerQueues + idxQueue;
	...
	if (!paTimers) return VERR_NO_PAGE_MEMORY;
...
static int vmm0_vmmr0_log_flusher(PVMR0 pvmr0)
{
	Sup::Vm &vm = *(Sup::Vm *)pvmr0;

	/*
	 * Stop vmmR3LogFlusher (VMMR3.cpp) thread immediately since we don't need it
	 * on Genode
	 */
	vm.vmm.s.hLogFlusherThread = NIL_RTTHREAD;
...
static void ioctl(SUPCALLVMMR0 &request)
{
	...
	case VMMR0_DO_PDM_QUEUE_CREATE:
		rc = vmmr0_pdm_queue_create(request.u.In.pVMR0, *(PDMQUEUECREATEREQ *)request.abReqPkt);
		return;
	...
	case VMMR0_DO_PGM_PHYS_ALLOCATE_RAM_RANGE:
		rc = vmmr0_pgm_phys_allocate_ram_range(request.u.In.pVMR0, *(PGMPHYSALLOCATERAMRANGEREQ *)request.abReqPkt);
		return;
	...
	case VMMR0_DO_PGM_PHYS_MMIO2_REGISTER:
		rc = vmmr0_pgm_phys_mmio2_register(request.u.In.pVMR0, *(PGMPHYSMMIO2REGISTERREQ *)request.abReqPkt);
		return;

	case VMMR0_DO_PGM_PHYS_ROM_ALLOCATE_RANGE:
		rc = vmmr0_pgm_phys_rom_allocate_range(request.u.In.pVMR0, *(PGMPHYSROMALLOCATERANGEREQ *)request.abReqPkt);
		return;

	case VMMR0_DO_TM_GROW_TIMER_QUEUE:
		/* vmr0, idxQueue, cNewTimers */
		rc = vmmr0_tm_grow_timer_queue(request.u.In.pVMR0, RT_HI_U32(request.u.In.u64Arg), RT_LO_U32(request.u.In.u64Arg));
		return;
	...
	case VMMR0_DO_VMMR0_LOG_FLUSHER:
		rc = vmm0_vmmr0_log_flusher(request.u.In.pVMR0);
		return;
