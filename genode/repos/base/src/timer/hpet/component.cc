...
class Timer::Hpet_device : Noncopyable, public Timer::Device
{
	private:
	...
	private:
		...
		Signal_handler<Hpet_device> _handler { _env.ep(), *this,
			                                     &Hpet_device::_handle_timeout };
	...
	public:
		...
		bool update_deadline(Deadline const abs) override
		{
			static auto const wrap_us = _convert_counter_to_us(_counter_max);

			if (abs.us == infinite_deadline.us)
				return true;
		...
		void notify() override { _handler.local_submit(); }
};


struct Timer::Main : Hpet_device::Wakeup_dispatcher
{
	Env &_env;
